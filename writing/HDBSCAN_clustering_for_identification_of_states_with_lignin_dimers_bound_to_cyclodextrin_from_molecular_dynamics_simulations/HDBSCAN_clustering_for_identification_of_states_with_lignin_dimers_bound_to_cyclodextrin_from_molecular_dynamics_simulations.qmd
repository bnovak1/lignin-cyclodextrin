...
![$\overrightarrow{CL}$ represented as a green arrow. In the molecular structures, gray atoms are carbon, red atoms are oxygen, and hydrogen atoms are not shown.](000_Attachments/fig-dnorm/dnorm.png){#fig-dnorm width=45%}
...
### Determination of `min_samples` {#sec-min-samples}
...
1. Find the last value of min_samples, $S$, where the number of clusters corresponding to $S+1$
...
The numbers of clusters as a function of min_samples are shown in @fig-nclusters-1, @fig-nclusters-2, and @fig-nclusters-3. The best values for min_samples and the corresponding numbers of clusters were determined using the algorithm described in [Determination of `min_samples`](#sec-min_samples).
...
```
![Number of clusters for dimer 1 as a function of `min_samples` for the HDBSCAN algorithm. The best value for `min_samples` and the corresponding number of clusters is circled in red and indicated in the legend.](000_Attachments/fig-nclusters_GG/min_samples.png){#fig-nclusters-1 width=75%}
...
os.chdir(current_dir)
```
![Number of clusters for dimer 2 as a function of `min_samples` for the HDBSCAN algorithm. The best value for `min_samples` and the corresponding number of clusters is circled in red and indicated in the legend.](000_Attachments/fig-nclusters_TGG/min_samples.png){#fig-nclusters-2 width=75%}
...
os.chdir(current_dir)
```
![Number of clusters for dimer 3 as a function of `min_samples` for the HDBSCAN algorithm. The best value for `min_samples` and the corresponding number of clusters is circled in red and indicated in the legend.](000_Attachments/fig-nclusters_GG_BB/min_samples.png){#fig-nclusters-3 width=75%}
...
For dimer 3, the cluster groups were named end-sec and center. The end-sec group consists of clusters where the distance for one of the lignin dimer ends (molecule is symmetric) is usually less than the distance for the other lignin dimer end and the lignin dimer COM and the COM of the lignin dimer is closer to the cyclodextrin secondary face than the cyclodextrin primary face (positive distance). The center group consists of clusters where the lignin dimer COM is located near the cyclodextrin COM. KDE plots of the cyclodextrin COM to lignin dimer normal distances (COM, ends) for each cluster group are shown in @fig-kde-3-1 and @fig-kde-3-2.
...
os.chdir(current_dir)
```
![KDE plots (Scott's Rule bandwidth) of the cyclodextrin to lignin dimer normal distances for dimer 3 for the center cluster group. In the legend, COM refers to the cyclodextrin COM to lignin dimer COM distance.](000_Attachments/fig-kde_GG_BB-center/cluster_distances_KDE_center.png){#fig-kde-3-2 width=75%}

## Proportion of configurations in each cluster and cluster group {#sec-cluster-prop}

For clusters and cluster groups containing configurations with a lignin dimer bound to the center of the cyclodextrin, the proportions of those configurations belonging to each cluster or cluster group were calculated. The proportions are shown in @tbl-cluster-prop-1, @tbl-cluster-prop-2, and @tbl-cluster-prop-3.
...
There are 4 cluster groups defined for dimer 2: head-sec, center-sec, tail-sec, and tail-pri (see [Dimer 2 cluster groups](#sec-cluster-groups-2)). The head-sec group consists of 2 clusters. However, most of the configurations belong to cluster 5. Cluster 4 accounts for about 2.3% of all configurations, while cluster 5 has about 71.5% of all configurations. The center-sec, tail-sec, and tail-pri groups each consist of a single cluster with about 2.8%, 23.0%, and 0.4% of all configurations, respectively.
...
Use of three relatively simple CVs increased the interpretability of the results and did not require dimensionality reduction.

Instead of DBSCAN, HDBSCAN clustering was used in this work to identify the clusters of configurations which made it easier to separate the configurations into separate clusters. HDBSCAN clustering was able to identify the configurations of the lignin dimer bound to the center of the cyclodextrin which were visually observed in the trajectories for all three dimers, as well as some subtypes of those main configuration types. In the previous analysis, there were visually observed configuration types which were not separated into their own clusters for two of the dimers.

When there were multiple subtypes for a main configuration type, the subtypes were grouped together since for each main configuration type there was always one dominant subtype. When looking at the proportions of configurations in each cluster group, the proportions were very similar between this work and the previous work for the dimer which had all of the main configuration types separated into their own clusters in the previous work, validating the results of this work.

Although three CVs is few enough to easily visualize the clusters, for computation of binding free energies it would be more efficient to use as few CVs as possible. In the previous work[@dean_complexation_2022], the binding free energies were computed using just the cyclodextrin COM to lignin dimer COM distance which is almost certainly not the best choice for a single CV. It would be useful to attempt to reduce the number of CVs to one or two using dimensionality reduction techniques such as UMAP[@mcinnes_umap_2020], SGOOP[@tiwary_spectral_2016], TICA[@perez-hernandez_identification_2013], etc. and check if the main configuration types can still be separated into their own clusters.

In addition to using an improved CV(s) for computation of binding free energies, the forcefield parameters for the lignin dimer and cyclodextrin might be improved to better match experimental results[@dean_complexation_2022] using OpenFF[@boothroyd_development_2023; @mcisaac_openforcefieldopenff-forcefields_2024] or other forcefields.
